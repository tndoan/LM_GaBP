    ...
    for i in nodes:
        for j in nodes:
            if graph[i].get(j) != None:
                mean_difference += abs(old_graph[i][j]['mean'] - graph[i][j]['mean'])
                mean_difference += abs(old_graph[i][j]['precision'] - graph[i][j]['precision'])
...
def iterate_until_convergence(graph, threshold, A):
    ...
    while isContinue and count < 5:
        ...
        #print graph.edge
        count += 1
        print count # print number of iteration
        print "======"
    return graph
